contact-manager
⁂Users can add, remove, block, activate, favorite, and search contacts in this dictionary-based Python contact manager.  The datetime module is used to store each contact's phone number, status, and last edited time.  User selections are handled by a menu-driven loop that shows all saved contacts and updates information whenever modifications are made.
<img width="368" height="647" alt="image" src="https://github.com/user-attachments/assets/8c9770fe-6f00-4023-b22d-8113259ca537" />
Objective:
The primary aim of this undertaking is to produce a functional menu-based application that highlights fundamental concepts of Python, such as:  Storing structured data (nested dictionaries and key-value pairs) using dictionaries, restructure business logic through function definitions, interacting with the user via input(), controlling flow with a while loop and if/elif/else on the main menu, and timestamping changes with the datetime module.
Requirements: Software Requirements: Python 3.x. Any IDE or code editor (for example, PyCharm or VS Code). Terminal or command prompt are examples of command line interfaces.Hardware Requirements: Any basic  computer system that can run Python.  A minimum of 2GB of RAM. A hard drive that can store project files and all Python installations.
Functional Requirements:
Add Contact:User enters a name and phone number.
System stores the contact with default status:
blocked = False
favorite = False
edited = current time
Remove Contact:
User provides the contact name.
System deletes the contact if it exists.
Block Contact:
Marks the contact as blocked.
Updates the "edited" timestamp.
Activate Contact:
Unblocks a previously blocked contact.
Updates the "edited" timestamp.
Favorite Contact:
Marks the contact as a favorite.
Updates the "edited" timestamp.
Search Contact:
User enters a name.
System displays all details of that contact if found.
Show All Contacts:
Displays all saved contacts with their phone numbers, statuses, and timestamps.
Exit Program:
Allows user to safely close the Contact Manager.
Output of my project:
<img width="342" height="612" alt="image" src="https://github.com/user-attachments/assets/1e4a4f35-81fd-453f-abfc-36de507f56b6" />
<img width="874" height="640" alt="image" src="https://github.com/user-attachments/assets/84885ba7-f8af-4676-876b-15033bedc0e6" />
<img width="369" height="684" alt="image" src="https://github.com/user-attachments/assets/f87782e2-6166-4fbf-8e6c-459ff8c50589" />




	
 
